...
public class ListPresenter extends AbstractPresenter<ListView> {
	...
	protected void personSelected(
			@Observes @EventQualifier(methodId = PERSON_SELECTED, viewInterface = ListView.class) ParameterDTO parameters) {
		view.showSelectedPersonDetails();
	}

	protected void editPerson(
			@Observes @EventQualifier(methodId = EDIT_PERSON, viewInterface = ListView.class) ParameterDTO parameters) {
		view.editSelectedPerson();
	}

	protected void savePerson(
			@Observes @EventQualifier(methodId = SAVE_PERSON, viewInterface = ListView.class) ParameterDTO parameters) {
	...
	protected void showAll(
			@Observes @EventQualifier(methodId = MainPresenter.SHOW_ALL, viewInterface = View.class) ParameterDTO parameters) {
		view.setPersonList(personDAO.listPeople());
		view.applyFilter(null);
	}

	protected void newContact(
			@Observes @EventQualifier(methodId = MainPresenter.NEW_CONTACT, viewInterface = View.class) ParameterDTO parameters) {
		view.editNewContact(personDAO.createNew());
	}

	protected void search(
			@Observes @EventQualifier(methodId = MainPresenter.SEARCH, viewInterface = View.class) ParameterDTO parameters) {
